...
class SemanticIdTokenizer(nn.Module):
    ...
    def __init__(
        ...
        rqvae_weights_path: str | None = None,
    ...
    def forward(self, batch: SeqBatch) -> TokenizedSeqBatch:
        ...
        # Preserve raw corpus item indices for the next (target) item so that
        # downstream losses (e.g. SASRec InfoNCE) can look up item embeddings.
        next_item_ids = batch.ids_fut.squeeze(-1) if batch.ids_fut is not None else None
