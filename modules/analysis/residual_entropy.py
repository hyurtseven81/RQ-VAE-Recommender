...
def compute_residual_stats(
    ...
    with torch.no_grad():
        for start in range(0, len(item_features), batch_size):
            ...
            output = rqvae.get_semantic_ids(batch)  # type: ignore[operator]
            ...
            for lvl in range(L):
                norms = residuals[lvl].norm(dim=0).cpu().numpy()  # [B]
                if sem_ids.ndim == 2:
                    # Shape is [L, B]
                    ids = sem_ids[lvl].cpu().numpy()
                else:
                    # Shape is [B, L]
                    ids = sem_ids[:, lvl].cpu().numpy()
                for i in range(B):
...
def run_analysis(
    ...
    output_path: str,
    alpha_schedule: list[float] | None = None,
    batch_size: int = 256,
    device: str = "cpu",
) -> pd.DataFrame:
