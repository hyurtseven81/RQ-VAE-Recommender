...
import polars as pl
import torch
from torch_geometric.data import HeteroData, InMemoryDataset, download_google_url, extract_zip
...
class AmazonReviews(InMemoryDataset, PreprocessingMixin):
    ...
    def train_test_split(self, max_seq_len=20):
        ...
        user_ids = []
        with open(
            os.path.join(self.raw_dir, self.split, "sequential_data.txt")
        ) as f:
            for line in f:
    ...
    def process(self, max_seq_len=20) -> None:
        ...
        with open(os.path.join(self.raw_dir, self.split, "datamaps.json")) as f:
